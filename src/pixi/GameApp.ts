...
export class GameApp {
  ...
  constructor() {
    ...
    this.game = createInitialGameState('warrior'); // 默认初始状态
  ...
  async init(canvas?: HTMLCanvasElement) {
    ...
    if (gameCanvas) {
      initOpts.canvas = gameCanvas;
    }

    await this.app.init(initOpts);

    // 浏览器环境：挂载到 DOM
    if (!gameCanvas && typeof document !== 'undefined') {
      const root = document.getElementById('root');
      if (root) root.appendChild(this.app.canvas as HTMLCanvasElement);
    }
  ...
  resize() {
    ...
    this.app.renderer.resize(720 * scale, 1280 * scale);
