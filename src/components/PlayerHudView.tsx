    ...
    <div className="relative z-[5] player-hud-panel">
      ...
      <div className="px-3 py-1">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          {/* v0.5 新状态显示 */}
          {(game.scarStacks || 0) > 0 && <BuffTooltip label={`伤痕×${game.scarStacks}`} icon={<PixelSword size={1.5} />} color="rgb(255,120,80)" bgColor="rgba(255,80,40,0.15)" borderColor="rgba(255,80,40,0.4)" title={`伤痕 ×${game.scarStacks}`} desc="战士自伤时累加伤痕层数。每层提供额外伤害加成。每回合衰减1层。" variant="debuff" />}
          {game.bloodChainTarget && <BuffTooltip label="血锁链" icon={<PixelSkull size={1.5} />} color="rgb(200,40,40)" bgColor="rgba(160,20,20,0.15)" borderColor="rgba(160,20,20,0.4)" title="血锁链" desc="锁定目标敌人，对其造成的伤害会传递给其他敌人。" variant="buff" />}
          {(game.soloSealTurnsLeft || 0) > 0 && <BuffTooltip label={`单挑${game.soloSealTurnsLeft}回合`} icon={<PixelSword size={1.5} />} color="rgb(255,200,60)" bgColor="rgba(212,160,48,0.15)" borderColor="rgba(212,160,48,0.4)" title={`单挑封印 ${game.soloSealTurnsLeft}回合`} desc="封印一个敌人进行单挑，期间对该敌人伤害大幅提升。" variant="buff" />}
          {(game.berserkTurnsLeft || 0) > 0 && <BuffTooltip label={`狂暴${game.berserkTurnsLeft}回合`} icon={<PixelFlame size={1.5} />} color="rgb(255,60,20)" bgColor="rgba(255,40,10,0.2)" borderColor="rgba(255,40,10,0.5)" title={`狂暴 ${game.berserkTurnsLeft}回合`} desc="攻击力大幅提升，但受到的伤害也会增加。" variant="buff" />}
