...
class Dialog extends pixi_js.Container {
  /**
  ...
  constructor(options) {
    ...
    this.onClose = new typedSignals.Signal();
    this.backdrop = new pixi_js.Container();
    this.contentView = new pixi_js.Container();
    this.buttonContainer = new List.List({
      type: "horizontal",
      elementsMargin: 10,
      ...options.buttonList
    });
  ...
  initBackdrop() {
    if (this.options.backdrop) {
      this.backdrop = view.getView(this.options.backdrop);
    } else {
  ...
  initInnerView() {
    ...
    if (nineSliceSprite) {
      if (typeof background === "string") {
        this.innerView = new pixi_js.NineSliceSprite({
          texture: pixi_js.Texture.from(background),
          leftWidth: nineSliceSprite[0],
          topHeight: nineSliceSprite[1],
          rightWidth: nineSliceSprite[2],
          bottomHeight: nineSliceSprite[3]
        });
      } else if (background instanceof pixi_js.Texture) {
        this.innerView = new pixi_js.NineSliceSprite({
          texture: background,
          leftWidth: nineSliceSprite[0],
          topHeight: nineSliceSprite[1],
        ...
        this.innerView = view.getView(background);
      }
    } else {
      this.innerView = view.getView(background);
    }
    if (this.options.width && this.options.height) {
      if (this.innerView instanceof pixi_js.NineSliceSprite) {
        this.innerView.width = this.options.width;
        this.innerView.height = this.options.height;
      } else if (this.innerView instanceof pixi_js.Graphics) {
        this.innerView.width = this.options.width;
        this.innerView.height = this.options.height;
      }
    }
  ...
  initTitle() {
    ...
    this.titleText = text.getTextView(this.options.title);
  ...
  initContent() {
    ...
    if (typeof this.options.content === "string" || typeof this.options.content === "number") {
      const textView = text.getTextView(this.options.content);
  ...
  initButtons() {
    ...
    buttonConfigs.forEach((btn, index) => {
      let button;
      switch (true) {
        case btn instanceof Button.Button:
          btn.onPress.connect(() => {
          ...
          break;
        case btn instanceof FancyButton.FancyButton:
          btn.onPress.connect(() => {
          ...
          break;
        default:
          button = new FancyButton.FancyButton(btn);
  ...
  open() {
    ...
    new tweedle_js.Tween(this.backdrop).to({ alpha: this.options.backdropAlpha ?? 0.5 }, duration).start();
    new tweedle_js.Tween(this.innerView.scale).to({ x: 1, y: 1 }, duration).start();
  }
  ...
  close() {
    ...
    new tweedle_js.Tween(this.backdrop).to({ alpha: 0 }, duration).start();
    new tweedle_js.Tween(this.innerView.scale).to({ x: 0.8, y: 0.8 }, duration).onComplete(() => {
      this.visible = false;
      this._isOpen = false;
      this.onClose.emit();
    }).start();
